...
//		}
	...
	@Auth
	@RequestMapping(value = "price/sent/find", method = RequestMethod.GET)
	...
	public BaseResp userInfo(StationUserDTO dto) {
		...
		try {
			User user = userService.findByMobOrName(dto);
	...
	@ResponseBody
	public BaseResp userList(String sellerId) {
		logger.info("# method : getStationInfo() [req]: " + sellerId);
		if (StringUtil.isNull(sellerId)) {
			logger.warn("# getStationInfo() : sellerId is null");
			return new BaseResp<>(ErrCode.VALIDATE_FAILED, "sellerId is null");
		}
		try {
			List<StationUser> list = stationService.listStationUser(sellerId);
			baseResp.setDetail(list);
		} catch (Exception e) {
			logger.error("获取小站信息异常:{}", e);
			baseResp = new BaseResp<>(ErrCode.ACCESS_DB_FAILED, "服务器异常");
		}
